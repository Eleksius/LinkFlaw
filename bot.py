...
async def cmd_start(message: Message):
    ...
    kb = admin_main_kb() if is_adm else main_kb()
...
async def on_new_member(event: ChatMemberUpdated):
    if event.invite_link and event.invite_link.invite_link:
        ...
        logger.info(
            f"New join via link {event.invite_link.invite_link} "
            f"by user {event.new_chat_member.user.id}"
        )
...
async def set_bot_commands():
    ...
    user_commands = [
        ...
        BotCommand(command="links",       description="Мои ссылки"),
...
async def main():
    await init_db()
    from db.models import migrate_db
    await migrate_db()
